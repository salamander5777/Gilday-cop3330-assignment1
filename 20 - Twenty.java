...
class Main {
  public static void main(String[] args) {
    ...
    double tax = 1;
    //(O*.055)
    if(input_st.equals("Wisconsin")){ //If statement will add tax to total if the state input is 'Wisconsin'.
    ...
    if((input_st.equals("Illinois"))){ //If statement will add tax to total if the state input is 'Illinois'.
      tax = (.08*O);
      to = Math.round((tax+O)*100.00)/100.00;
    }
